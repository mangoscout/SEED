...
const float pi_local = 3.14159f; // Define a local variable for Pi value

// I2C Communication Configuration
const uint8_t I2C_SLAVE_ADDR = 0x08; // Arduino I2C address
const uint8_t I2C_CMD_ID = 0x01;// Command ID to differentiate between different commands sent by Pi

// Variables to hold received data
volatile bool i2c_new_cmd = false; // Flag to indicate new command received from Pi
volatile uint8_t i2c_lw = 0; // Left wheel control (0 or 1)
volatile uint8_t i2c_rw = 0; // Right wheel control (0 or 1)
...
// Encoder variables
volatile long encoderPos[2] = {0, 0}; // Encoder position counters for left and right wheels
...
const int enc1PinA = 2; // LEFT encoder A (interrupt capable)
const int enc1PinB = 5; // LEFT encoder B
const int enc2PinA = 3; // RIGHT encoder A (interrupt capable)
const int enc2PinB = 6; // RIGHT encoder B

// Pin assignments for motor control
const int shieldPin = 4; // Motor driver enable pin
const int mot1Dir = 8; // LEFT motor direction pin
const int mot2Dir = 7; // RIGHT motor direction pin
const int mot1Pow = 10; // LEFT motor PWM control pin
const int mot2Pow = 9; // RIGHT motor PWM control pin
...
void loop() {
  ...
  for (i = 0; i < 2; i++) {
    ...
    integral_error[i] = constrain(integral_error[i], -I_MAX, I_MAX);  // Prevent integrator windup

    desired_speed[i] = Kp_pos * pos_error[i] + Ki_pos * integral_error[i];  // Calculate desired speed for each wheel
    desired_speed[i] = constrain(desired_speed[i], -MAX_DESIRED_SPEED, MAX_DESIRED_SPEED);  // Limit desired speed
  }
  ...
  for (i = 0; i < 2; i++) {
    ...
    voltage[i] = constrain(Kp_velocity * velocity_error[i], -8.2f, 8.2f); // Calculate motor voltage
  }
  ...
  analogWrite(mot1Pow, constrain(PWM[0], 0, 255)); // Apply PWM to LEFT motor
  ...
  analogWrite(mot2Pow, constrain(PWM[1], 0, 255)); // Apply PWM to RIGHT motor
